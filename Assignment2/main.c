	...
	uint8_t *cdb, uint8_t direction, int data_length, uint32_t *ret_tag)
{
	...
	*ret_tag = tag;
	...
	memcpy(cbw->CBWCB, cdb, cdb_len);
	    ...
	    printk(KERN_INFO"Sent %d CDB bytes into CBW\n", cdb_len);
	    printk(KERN_INFO"sent %d bytes of CBW\n",size);
...
static int test_mass_storage (struct usb_device *device , uint8_t endpoint_in, uint8_t endpoint_out)
{
	...
	printk(KERN_INFO"Value of &buffer[0] is %d\n",buffer[0]);
	max_lba = be_to_int32(&buffer[0]);
	block_size = be_to_int32(&buffer[4]);
	device_size = ((long)(max_lba+1))*block_size/(1024*1024);
	printk("Max_lba: %x\nBlock_size found: %x\n",max_lba,block_size);
	printk("Device size found : %ld MB \n\n",device_size);
...
{   struct usb_device *device;
    ...
    if(id->idProduct == USB_PID && id->idVendor==USB_VID)
	{
		printk(KERN_INFO "“USB drive detected is known”\n");
	}
	printk(KERN_INFO "\nReading device descriptor:\n");
	printk(KERN_INFO "USB VID: %04x\n",id->idVendor);
	printk(KERN_INFO "USB PID: %04x\n",id->idProduct);
	...
	printk(KERN_INFO "Number of interfaces: %d\n", nb_ifaces);

...
#define create_alignment _IOW(MAGIC_NUM, ALLIGNMENT, unsigned int)
...
static ssize_t adc_read(struct file *f, char __user *buf, size_t len, loff_t *off)
{
	...
	get_random_bytes(&data_adc, 2);
	data_adc=data_adc%10;

	printk(KERN_INFO "%d\n",data_adc);
	printk(KERN_INFO "ADC_Value: %d", data_adc);
	printk(KERN_INFO "Channel : %d \n",channel);
	printk(KERN_INFO "Allignment : %d \n",allignment);
	if(allignment == 1)
		data_adc = data_adc << 6;
	copy_to_user(buf, &data_adc, 2);
	return 0;
}
...
MODULE_DESCRIPTION(" First_ADC_driver");
